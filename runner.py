...
def main():
    # ВАЖНО: НЕ удаляем очередь апдейтов на каждом запуске
    ...
    for upd in updates:
        ...
        # /ids — только команда (её бот получит даже при privacy ON)
        ...
        # источник: группа+тема (только если state есть и ids заданы)
